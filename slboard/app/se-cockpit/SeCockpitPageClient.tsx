...
export function SeCockpitPageClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const json = await readApiJsonOk<ApiSeCockpit>(res, 'Steuerungs-Cockpit konnte nicht geladen werden.');
      setData(json);
    } catch (e) {
      setData(null);
      setError(serializeApiError(e, 'Steuerungs-Cockpit konnte nicht geladen werden.'));
    } finally {
      setLoading(false);
    }
  }, []);
